...
function Strategy(options, verify) {
  ...
  this._apiVersion = options.apiVersion || '5.107'; // least required for getting city value
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.getProtectedResource(url, accessToken, function (err, body, res) {
    ...
    try {
      ...
      if (parsedBody.error) {
        const error = new VkontakteAPIError(parsedBody.error.error_msg, parsedBody.error.error_code)
