...
const VkontakteTokenError = require('./errors/vkontaktetokenerror.js')
const VkontakteAPIError = require('./errors/vkontakteapierror.js')
const { parse } = require('./profile.js')
 ...
 * @param {number} [options.photoSize]
 * @param {string} [options.authorizationURL]
 * @param {string} [options.tokenURL]
 * @param {string} [options.scopeSeparator]
 * @param {string} [options.profileURL]
 * @param {string[]} [options.profileFields]
 * @param {string} [options.apiVersion]
 * @param {function} verify
 * @api public
 */

class VkontakteStrategy extends OAuth2Strategy {
  constructor ({ lang, photoSize, ...options } = {}, verify) {
    lang ??= 'en'
    photoSize ??= 200
    ...
    super(options, VkontakteStrategy.verifyWrapper(options, verify))
    ...
    this._apiVersion = options.apiVersion || '5.131'
  }
  ...
  userProfile (accessToken, done) {
    ...
    url +=
      '?fields=' + fields.join(',') + '&v=' + this._apiVersion + '&https=1'
    ...
    this._oauth2.getProtectedResource(
      ...
      function (err, body) {
        ...
        try {
          ...
          if (parsedBody.error) {
            const error = new VkontakteAPIError(
              parsedBody.error.error_msg,
              parsedBody.error.error_code
            )

            return done(error)
          }

          const json = parsedBody.response[0]
   ...
   */
  static verifyWrapper (options, verify) {
    return function passportVerify (
      ...
      if (verify.length === 6) {
        ...
        this.error(
          new Error(
            'VKontakteStrategy: verify callback must take 6/5/4 parameters'
          )
        )
      }
    }
